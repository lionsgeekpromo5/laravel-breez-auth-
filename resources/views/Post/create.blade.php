<x-app-layout>

    <section class="flex-grow container mx-auto p-6">
        <h1 class="text-3xl font-bold text-gray-800 mb-6 text-center">Create a New Blog Post</h1>
        
        <form action="{{ route('post.store') }}" method="POST" class="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow-md">
            <!-- Title -->
            @csrf
            <div class="mb-4">
                <label for="title" class="block text-gray-700 font-semibold mb-2">Title</label>
                <input type="text" id="title" name="post_title" required
                       class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                       placeholder="Enter blog title">
            </div>
            <!-- Title -->
            <div class="mb-4">
                <label for="subtitle" class="block text-gray-700 font-semibold mb-2">Sub Title</label>
                <input type="text" id="subtitle" name="post_subtitle" required
                       class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                       placeholder="Enter blog title">
            </div>

            <!-- description -->
            <div class="mb-4">
                <label for="description" class="block text-gray-700 font-semibold mb-2">Description</label>
                <textarea id="description" name="post_description" required 
                          class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                          placeholder="Write your blog content here"></textarea>
            </div>

            <!-- Submit Button -->
            <div class="text-center">
                <button type="submit"
                        class="bg-blue-600 text-white px-6 py-2.5 rounded-md font-semibold hover:bg-blue-700 transition">
                    Publish Post
                </button>
            </div>
        </form>
    </section>

</x-app-layout>